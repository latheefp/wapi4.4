...
<div class="row" data-select2-id="15">
    <div class="form-group col-xl-6">
        <label>Name *</label>
        <input type="text" class="form-control" name="name" id="name" required="" placeholder="Name" value=<?= $user->name ?>>
    </div>
    ...
    <?php
    $group_id = null;
    if (isset($user->ugroup_id)) {
        $group_id = $user->ugroup_id;
    }
   ...
   // debug($filter_array);
    ...
    <div class="form-group col-xl-3">
        <label>Notes</label>
        <div>
            <textarea placeholder="Remarks" name="remarks" class="form-control" rows="2"><?= $user->remarks ?></textarea>
        </div>
    </div>
    ...
    <div class="form-group col-xl-3" data-select2-id="37">
        ...
        <select class="form-control" required="" name="active" data-select2-id="10" tabindex="-1" aria-hidden="true">
            <option value="1" <?php echo ($user->active == 1) ? "selected" : ''; ?> data-select2-id="12">Enabled</option>
            <option value="0" data-select2-id="38" <?php echo ($user->active == 1) ? '' : 'selected'; ?>>Disabled</option>
        </select>
    </div>
    <div class="form-group col-xl-3">
        <label>Login *</label>
        <input type="text" class="form-control" name="username" id="username" required="" autocomplete="off" placeholder="Username" value=<?= $user->username ?>>
    </div>
</div>
...
//         alert ("Validaty is true");
    ...
    function ajaxvalidate(event) {
        ...
        $.ajax({
            ...
            url: "/ajaxes/validate/Users/edit",
            method: "POST",
//  async:false,
            data: form.serialize(),
